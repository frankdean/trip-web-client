...
module.exports = {
  ...
  keySequenceForSafariDate: keySequenceForSafariDate,
  keySequenceForSafariDateTime: keySequenceForSafariDateTime,
  wait: wait,
 ...
 * language settings.
 *
 * Note: this implementation makes assumptions about the format
 * required in the browser and has not been validated for any
 * languages other than en_GB.UTF-8.  It will need updating on a
 * case-by-case basis where the implementation is incorrect.
 *
 * @param {string} day the day of the month
 * @param {string} month the day of the month
 * @param {string} year the day of the month
 */
function keySequenceForSafariDate(year, month, day) {
  var mdy = new RegExp(mdyRegexCountries);
  var ymd = new RegExp(ymdRegexCountries);
  if (process.env.LANG == null || mdy.test(process.env.LANG)) {
    return month.padStart(2, '0') + '\t' + day.padStart(2, '0') + '\t' + year;
 ...
 * @param {string) second the second of the minute
 */
function keySequenceForSafariDateTime(year, month, day, hour, minute, second) {
  var am = true,
      h = Number(hour),
      h12 = h;
  ...
  var retval = keySequenceForSafariDate(year, month, day, hour);
  if (process.env.LANG == null || h12Regex.test(process.env.LANG)) {
    if (h >= 12) {
      am = false;
      if (h > 12) {
